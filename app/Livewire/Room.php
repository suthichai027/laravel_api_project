...
class Room extends Component
{
    ...
    {
        $this->rooms = RoomModel::where('status', 'use')->orderBy('id', 'asc')->get();
    }
    ...
    public function createRoom()
    {
        ...
        if ($this->from_number > $this->to_number) {
            $this->addError('from_number', 'เลขที่เริ่มต้นต้องน้อยกว่าเลขที่สิ้นสุด');
            return;
        }

        if ($this->to_number > 1000) {
            $this->addError('to_namber', 'เลขที่สิ้นสุดต้องน้อยกว่า 1000');
            return;
        }
